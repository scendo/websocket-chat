...
module.exports = function(io, socket) {
  ...
  socket.on(ROOM_CREATE, async ({ group, name, users }, callback) => {
    ...
    const roomMetas = users.reduce((array, userId) => {
      return [
        ...array,
        {
          userId,
          key: UserMeta.getRoomMetaKey(newRoom.id),
          value: {
    ...
    //create the room
    const userUpdatesPromises = users.reduce((array, userId) => {
      return [...array, User.joinRoom(userId, newRoom.id)];
    }, []);

    const result = newRoom.save().catch(e => console.log(e));
  ...
  socket.on(ROOM_OPEN, async (data, callback) => {
    ...
    if (room) {
      ...
      /**
       * Direct rooms track unread messages.
       * reset the unread message and total unread message counts for the direct room
       */
